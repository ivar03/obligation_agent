      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 flex items-center gap-3">
        ...
        {searchTraceId.trim() && (
          <Link
            ...
            className="px-3 py-1 bg-cyan-600 hover:bg-cyan-500 text-white rounded text-xs font-semibold flex items-center gap-1 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
          ...
          <div className="text-2xl font-bold text-slate-100 mt-2 font-mono">
            ...
            <span className="text-xs font-normal text-slate-400 ml-1">inbox</span>
          </div>
          <div className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
            DLQ Depth:{" "}
            <span
              className={`font-semibold ${(data?.dead_letter_depth || 0) > 0 ? "text-rose-400" : "text-slate-400"}`}
            >
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
          ...
          <div className="text-2xl font-bold text-amber-400 mt-2 font-mono">
            ...
            <span className="text-xs font-normal text-slate-400 ml-1">alerts</span>
          </div>
          <div className="text-[11px] text-slate-500 mt-1">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono">
                {(data?.slos || []).map((s, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/20">
                    <td className="py-2.5 font-sans font-medium text-slate-200">{s.name}</td>
                    <td className="py-2.5 text-slate-400">{s.target_percentage}%</td>
          ...
          <div className="space-y-3">
            {(data?.error_budgets || []).map((b, idx) => (
              <div key={idx} className="p-3 rounded-lg bg-slate-950 border border-slate-800 text-xs space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-slate-200">{b.slo_name}</span>
                ...
                <div className="text-[11px] text-slate-500">{b.status_reason}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(data?.provider_health_summary || {}).map(([p, state]) => (
            <div key={p} className="p-3 rounded-lg bg-slate-950 border border-slate-800 text-xs">
              <span className="text-[10px] uppercase text-slate-500 block font-mono">Provider</span>
