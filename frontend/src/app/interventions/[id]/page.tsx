    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-zinc-400">Loading intervention audit records...</p>
        </div>
      </div>
    );
  }
  ...
  if (!intervention) {
    return (
      <div className="text-center py-16 space-y-4">
        <h2 className="text-xl font-bold text-white">Intervention Not Found</h2>
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-semibold transition-colors"
        >
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="p-2 rounded-xl border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 transition-colors"
            title="Go Back"
          >
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-400">ID: {intervention.id.slice(0, 8)}...</span>
            </div>
        ...
        <div className="flex items-center gap-2.5">
          {intervention.status === "APPROVED" && (
            <button
              ...
              className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-all shadow-md shadow-blue-600/20 active:scale-95 flex items-center gap-1.5"
            >
          ...
          {intervention.status !== "RESOLVED" && intervention.status !== "CANCELLED" && (
            <button
              ...
              className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 text-xs font-bold transition-all shadow-md shadow-amber-500/20 active:scale-95 flex items-center gap-1.5"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-5 space-y-3 shadow-md">
            <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-blue-400" />
              <span>Intervention Rationale & Grounding</span>
            </h2>
            <p className="text-sm text-zinc-200 leading-relaxed">{intervention.rationale}</p>
            <div className="pt-2 flex flex-wrap items-center gap-4 text-xs text-zinc-400 border-t border-zinc-800/80">
              <span>
                Target Owner: <strong className="text-white">{intervention.target_owner}</strong>
              </span>
              <span>
                Beneficiary: <strong className="text-white">{intervention.target_beneficiary}</strong>
              </span>
              <span>
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-5 space-y-4 shadow-md">
            <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2">
            ...
            <div className="space-y-3">
              ...
              {intervention.approved_message && intervention.approved_message !== intervention.message_draft && (
                <div className="bg-zinc-950 border border-emerald-500/30 rounded-xl p-4 space-y-1.5">
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-5 space-y-4 shadow-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-zinc-400 block mb-1">Outcome Status</label>
                <select
                  value={selectedOutcome}
                  onChange={(e) => setSelectedOutcome(e.target.value as InterventionOutcome)}
                  className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:border-blue-500"
                >
              ...
              <div>
                <label className="text-xs font-semibold text-zinc-400 block mb-1">Notes / Context (Optional)</label>
                <input
                  type="text"
                  value={outcomeNotes}
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-semibold transition-colors disabled:opacity-50 flex items-center gap-1.5"
              >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-5 space-y-3.5 shadow-md">
            <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-400">Execution State</h2>
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-zinc-800/80">
                <span className="text-zinc-400">Status:</span>
                <span className="font-semibold text-white">{intervention.status}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-zinc-800/80">
                <span className="text-zinc-400">Execution Mode:</span>
                <span className="font-mono text-zinc-300">{intervention.execution_mode}</span>
              </div>
              {intervention.execution_reference && (
                <div className="flex justify-between py-1 border-b border-zinc-800/80">
                  <span className="text-zinc-400">Reference:</span>
                  <span className="font-mono text-blue-400">{intervention.execution_reference}</span>
                </div>
              )}
              {intervention.approved_at && (
                <div className="flex justify-between py-1 border-b border-zinc-800/80">
                  <span className="text-zinc-400">Approved:</span>
                  <span className="text-zinc-300">{new Date(intervention.approved_at).toLocaleString()}</span>
                </div>
              )}
              {intervention.executed_at && (
                <div className="flex justify-between py-1 border-b border-zinc-800/80">
                  <span className="text-zinc-400">Executed:</span>
                  <span className="text-zinc-300">{new Date(intervention.executed_at).toLocaleString()}</span>
                </div>
              )}
              {intervention.scheduled_for && (
                <div className="flex justify-between py-1 border-b border-zinc-800/80">
                  <span className="text-zinc-400">Scheduled For:</span>
            ...
            <div className="pt-2">
              <Link
                ...
                className="w-full px-3 py-2 rounded-xl bg-zinc-950 hover:bg-zinc-800 border border-zinc-800 text-xs font-semibold text-zinc-200 transition-colors flex items-center justify-center gap-1.5"
              >
            ...
            <div className="relative pl-4 space-y-4 border-l border-zinc-800">
              {(intervention.audit_trail || []).map((entry, idx) => (
                <div key={idx} className="relative space-y-1">
                  <div className="absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full bg-blue-500 ring-4 ring-zinc-900" />
                  ...
                  <p className="text-[11px] text-zinc-400">
                    Actor: <strong className="text-zinc-300">{entry.actor}</strong>
                  </p>
                  {entry.details && Object.keys(entry.details).length > 0 && (
                    <pre className="text-[10px] text-zinc-400 bg-zinc-950 p-2 rounded-lg border border-zinc-800/80 overflow-x-auto">
