...
function AcceptInvitationContent() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white border border-stone-200 rounded-2xl p-8 shadow-md">
        ...
        {success ? (
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 text-center space-y-3">
            ...
            <h3 className="text-base font-semibold text-emerald-900">Invitation Accepted!</h3>
            <p className="text-xs text-stone-700">
              You have joined workspace <span className="font-mono text-emerald-700 font-bold">{acceptedWorkspaceId}</span>.
          ...
          <form onSubmit={handleAccept} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full bg-stone-50 border border-stone-200 rounded-xl px-3 py-2 text-xs text-stone-800 focus:outline-none focus:border-orange-500 font-mono"
              />
            </div>

            {!isAuthenticated ? (
              <>
                <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl text-xs text-stone-700 flex items-start gap-2">
              ...
              <div className="p-3 bg-orange-50 border border-orange-200 rounded-xl text-xs text-orange-800">
                Logged in as <span className="font-semibold text-orange-900">{user?.email}</span> ({user?.display_name}). This account will be linked to the workspace.
              </div>
            )}
            ...
            <button
              ...
              className="w-full bg-orange-600 hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-semibold py-2.5 rounded-xl shadow transition-colors flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
