    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-200 pb-6">
        ...
        <button
          onClick={fetchData}
          disabled={loading}
          className="self-start sm:self-auto inline-flex items-center gap-2 px-4 py-2 bg-stone-200 hover:bg-stone-300 text-stone-800 rounded-lg text-sm font-medium transition-colors border border-stone-300"
        >
      ...
      {activeTab === "ROOT_CAUSE" && (
        <div className="space-y-8">
          ...
          {phase14Loading ? (
            <div className="py-12 flex justify-center items-center text-stone-600 text-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-stone-50/60 rounded-xl border border-stone-200 space-y-3">
                    ...
                    {[...rootCauseData.upstream_causes, ...rootCauseData.direct_causes].length === 0 ? (
                      <div className="text-xs text-stone-500 italic">No direct blockers detected.</div>
                    ) : (
                  ...
                  <div className="p-4 bg-stone-50/60 rounded-xl border border-stone-200 space-y-3">
                    ...
                    {rootCauseData.contributing_factors.length === 0 ? (
                      <div className="text-xs text-stone-500 italic">No adverse contributing factors.</div>
                    ) : (
                      <div className="space-y-2">
              ...
              {impactData && (
                <div className="bg-stone-100/60 border border-stone-200 rounded-2xl p-6 shadow-sm space-y-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div className="p-4 bg-stone-50/60 rounded-xl border border-stone-200">
                      <div className="text-xs text-stone-600">Total Downstream</div>
                      <div className="text-2xl font-bold font-mono text-stone-900 mt-1">
                        {impactData.total_downstream_dependents_count}
                      </div>
                      <div className="text-[10px] text-stone-500">Dependents affected</div>
                    </div>

                    <div className="p-4 bg-stone-50/60 rounded-xl border border-stone-200">
                      <div className="text-xs text-stone-600">Max Dependency Depth</div>
                      <div className="text-2xl font-bold font-mono text-stone-900 mt-1">
                        {impactData.maximum_dependency_depth}
                      </div>
                      <div className="text-[10px] text-stone-500">Graph hops</div>
                    </div>

                    <div className="p-4 bg-stone-50/60 rounded-xl border border-stone-200">
                      <div className="text-xs text-stone-600">Affected Owners</div>
                      <div className="text-2xl font-bold font-mono text-stone-900 mt-1">
                        {impactData.affected_owners.length}
                      </div>
                      <div className="text-[10px] text-stone-500">Team members involved</div>
                    </div>
                  </div>
                  ...
                  {impactData.downstream_items && impactData.downstream_items.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-xs font-semibold text-stone-700">Affected Downstream Commitments:</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {impactData.downstream_items.map((item: { action?: string; owner?: string; hop_distance?: number; status?: string }, idx) => (
                          <div
                            key={idx}
                            className="p-3 bg-stone-50/80 rounded-lg border border-stone-200 flex items-center justify-between text-xs"
                          >
                            <div>
                              <div className="font-semibold text-stone-800">{item.action}</div>
                              <div className="text-[11px] text-stone-600">Owner: {item.owner} ({item.hop_distance} hop away)</div>
                            </div>
                            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-stone-200 text-stone-700 border border-stone-300">
                              {item.status}
                            </span>
                          </div>
                        ))}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-stone-600">Owner: {node.owner}</div>
                          </div>
                        </div>

                        <div className="flex items-center gap-3 self-end sm:self-auto font-mono text-xs">
                          <span className="px-2 py-0.5 rounded bg-stone-100 border border-stone-300 text-stone-700">
                            {node.status}
                          </span>
                    ...
                    {simulationResult && (
                      <div className="p-4 bg-indigo-950/30 border border-blue-600/40 rounded-xl space-y-3 animate-in fade-in duration-200">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2">
                          ...
                          <div className="p-2.5 bg-stone-50 rounded-lg border border-stone-200">
                            <div className="text-[10px] text-stone-600">Unblocked Dependents</div>
                ...
                <div className="bg-stone-100/60 border border-stone-200 rounded-2xl p-6 shadow-sm space-y-4">
                  ...
                  <div className="space-y-3">
                    {bottlenecks.bottlenecks.slice(0, 5).map((b) => (
                      <div
                        key={b.obligation_id}
                        className="p-4 bg-stone-50/60 rounded-xl border border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs"
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-stone-900">{b.action}</span>
                            <span className="px-1.5 py-0.2 rounded text-[10px] font-mono bg-stone-200 text-stone-600">
                              {b.status}
                            </span>
                          </div>
                          <p className="text-stone-600">{b.neutral_summary}</p>
                        </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-stone-100 border border-stone-200 rounded-lg p-0.5 text-xs">
          ...
          {filteredPredictions.length === 0 && (
            <div className="bg-stone-100/40 border border-stone-200/80 rounded-2xl p-12 text-center text-stone-600 space-y-2">
              <Brain className="w-8 h-8 text-stone-400 mx-auto" />
              <div className="text-base font-semibold text-stone-700">No matching forecasts</div>
              <div className="text-xs text-stone-500">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-stone-100/60 border border-stone-200 rounded-xl p-4">
              <div className="text-xs text-stone-600">Total Recorded Outcomes</div>
              <div className="text-2xl font-bold font-mono text-stone-900 mt-1">
            ...
            {owners.map((owner) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="px-2 py-0.5 rounded text-[11px] font-mono bg-stone-200 text-stone-700">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            {memoriesList.length === 0 ? (
              <div className="p-12 text-center text-xs text-stone-500 border border-dashed border-stone-200 rounded-2xl">
              ...
              <div className="grid grid-cols-1 gap-3">
                {memoriesList.map((item) => (
                  <div
                    key={item.memory.id}
                    className="p-4 bg-stone-100/60 border border-stone-200 rounded-xl hover:border-stone-300 transition space-y-2"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-stone-200 text-stone-700 border border-stone-300">
                    ...
                    {(item.memory.entities.length > 0 || item.memory.topics.length > 0) && (
                      <div className="flex flex-wrap gap-1.5 pt-2 border-t border-stone-200/60">
                        {item.memory.entities.map((e, idx) => (
                          <span
                            key={`e-${idx}`}
                            className="text-[10px] bg-stone-50 text-stone-600 px-1.5 py-0.5 rounded border border-stone-200"
                          >
                            entity:{e}
                          </span>
                        ))}
