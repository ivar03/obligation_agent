    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
        <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          ...
          <h2 className="text-lg font-bold text-white mb-2">Error Loading Decision Plan</h2>
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 border-b border-slate-800/80 gap-4">
        <div>
          <Link
            href="/intelligence/decisions"
            className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-violet-400 transition mb-2"
          >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {plan.status === "GENERATED" || plan.status === "PENDING_REVIEW" ? (
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-sm rounded-lg transition shadow-lg shadow-emerald-950/40"
              >
              ...
              <button
                ...
                disabled={actionLoading}
                className="flex items-center gap-1.5 px-4 py-2 bg-slate-800 hover:bg-rose-900/60 text-slate-200 hover:text-rose-200 border border-slate-700 font-medium text-sm rounded-lg transition"
              >
          ...
          <button
            ...
            disabled={actionLoading}
            className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-900 border border-slate-700 hover:bg-slate-800 text-slate-300 text-sm font-medium rounded-lg transition"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 my-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900/80 border border-slate-800/80 rounded-xl p-5 shadow-sm backdrop-blur-sm">
            ...
            <div className="bg-slate-950/60 border border-slate-800 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs text-slate-400">
                <div>
                  ...
                  <strong className="text-slate-200">{plan.target_obligation_owner || "Unassigned"}</strong>
                </div>
                <div>
                ...
                <React.Fragment key={node.obligation_id || idx}>
                  <div className={`p-3 rounded-lg border text-xs min-w-[170px] ${
                    ...
                    <div className="text-[10px] uppercase font-bold text-slate-400 mb-1 flex items-center justify-between">
                      ...
                      <span className="font-mono text-[9px] text-slate-500">{node.status}</span>
                    </div>
                    <div className="font-semibold truncate text-white" title={node.action}>
                      {node.action}
                    </div>
            ...
            <div className="space-y-3">
              {allStrategies.map((strat, idx) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] text-slate-400 bg-slate-950/80 p-2.5 rounded border border-slate-800/80">
                    <div>
                      <span className="text-slate-500 block">Target Owner</span>
                      <strong className="text-slate-200">{strat.target_owner}</strong>
                    </div>
                    <div>
                      <span className="text-slate-500 block">Expected Impact</span>
                      <span className="text-slate-200 font-medium">{strat.expected_impact}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block">Risk Reduction (Δ)</span>
                      <strong className="text-emerald-400 font-mono">{strat.risk_reduction?.toFixed(2)}</strong>
                    </div>
                    <div>
                      <span className="text-slate-500 block">Projected Unblocks</span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-b from-slate-900/90 to-slate-950/90 border border-violet-500/30 rounded-xl p-5 shadow-lg relative overflow-hidden">
            ...
            {plan.status === "APPROVED" || plan.status === "PARTIALLY_EXECUTED" ? (
                ...
                <div className="bg-slate-950/80 p-3 rounded-lg border border-slate-800 space-y-2 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <select
                      ...
                      className="bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded px-2 py-1 outline-none font-mono"
                    >
                  ...
                  <div className="flex items-center justify-between text-slate-400">
                    ...
                    <strong className="text-slate-200">{plan.recommended_actions?.target_owner || plan.target_obligation_owner || "Unassigned"}</strong>
                  </div>

                  <div className="space-y-1 pt-1">
                    <span className="text-[11px] text-slate-500 block">Outbound Payload Snippet</span>
                ...
                <div className="flex items-center gap-2">
                  {!latestExecution || ["PENDING_AUTHORIZATION", "AUTHORIZED"].includes(latestExecution.status) ? (
                    <button
                      ...
                      disabled={actionLoading}
                      className="flex-1 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-semibold text-xs rounded-lg transition shadow-md shadow-violet-950/40 flex items-center justify-center gap-1.5"
                    >
                    ...
                    <button
                      ...
                      disabled={actionLoading}
                      className="flex-1 py-2 bg-violet-600 hover:bg-violet-500 text-white font-semibold text-xs rounded-lg transition flex items-center justify-center gap-1.5"
                    >
                  ...
                  {latestExecution && (
                    <button
                      ...
                      className="px-3 py-2 bg-slate-900 border border-slate-700 hover:bg-slate-800 text-slate-300 text-xs font-semibold rounded-lg transition flex items-center gap-1"
                    >
              ...
              <div className="text-xs text-slate-500 italic p-3 bg-slate-950/50 rounded-lg border border-slate-800/80">
                Decision Plan must be <strong>APPROVED</strong> by operator before controlled execution can be dispatched.
              </div>
            )}

            {/* Invariant Note */}
            <div className="mt-3 text-[10px] text-slate-500 leading-normal border-t border-slate-800/80 pt-2 flex items-center gap-1.5">
            ...
            {plan.human_decisions_required?.length > 0 ? (
              <div className="space-y-2.5">
                {plan.human_decisions_required.map((dec, idx) => (
                  <div key={idx} className="bg-slate-950/80 border border-slate-800 rounded-lg p-3 text-xs">
                    ...
                    <p className="text-slate-300 mb-1.5">{dec.reason}</p>
              ...
              <div className="text-xs text-slate-500 italic">No special authorization required.</div>
            )}
          </div>
          ...
          <div className="bg-slate-900/80 border border-slate-800/80 rounded-xl p-5 shadow-sm backdrop-blur-sm">
            ...
            <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3 text-xs text-slate-300 font-mono whitespace-pre-wrap leading-relaxed">
          ...
          <div className="bg-slate-900/80 border border-slate-800/80 rounded-xl p-5 shadow-sm backdrop-blur-sm">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
              <Clock className="w-4 h-4 text-slate-400" /> Immutable Version History ({history.length})
            </h2>
            <div className="space-y-2">
              {history.map((h) => (
                ...
                >
                  <div>
                    <div className="font-semibold text-white">Plan Version v{h.plan_version}</div>
      ...
      {showApproveModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-base font-bold text-white mb-2">Authorize & Approve Decision Plan</h3>
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs text-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none mb-4"
              rows={3}
            />
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-base font-bold text-white mb-2">Reject Decision Plan</h3>
            <p className="text-xs text-slate-400 mb-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-white">Immutable Execution Receipt</h3>
              </div>
              <button
            ...
            <div className="space-y-2.5 text-xs">
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1.5 font-mono">
                ...
                <div className="flex justify-between text-slate-400">
                  <span>Provider:</span>
                  <span className="text-slate-200 uppercase">{executionReceipt.provider}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>Delivery Status:</span>
                  <span className="text-slate-200">{executionReceipt.delivery_status}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>Authorized By:</span>
                  <span className="text-slate-200">{executionReceipt.authorized_by || "Operator"}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                ...
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="text-slate-200">{executionReceipt.retry_count}</span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
              <Link
                ...
                className="px-3.5 py-1.5 bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold rounded-lg transition"
              >
